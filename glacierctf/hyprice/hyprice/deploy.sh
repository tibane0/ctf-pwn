...
build_container() {
  ...
  docker build -t localhost/chall-hyprice --platform linux/amd64 --pull=true   .
}
...
run_container() {
  ...
  docker run --name chall-hyprice --rm -p 127.0.0.1:1337:1337 -e HOST=127.0.0.1 -e PORT=1337 -e TIMEOUT=180 --read-only --privileged --pull=never --platform linux/amd64 localhost/chall-hyprice
}

kill_container() {
  docker ps --filter "name=chall-hyprice" --format "{{.ID}}" |
    tr '\n' ' ' |
